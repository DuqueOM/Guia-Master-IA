...
- Visualización de optimización: `Herramientas_Estudio/VISUALIZACION_GRADIENT_DESCENT.md`
- Simulacros: `Herramientas_Estudio/SIMULACRO_EXAMEN_TEORICO.md`
- Evaluación (rúbrica): [Herramientas_Estudio/RUBRICA_v1.md](../Herramientas_Estudio/RUBRICA_v1.md) (scope `M03` en `rubrica.csv`)
- Protocolo completo:
...
|----------|---------|------------------------------|----------|
| **Obligatorio** | `Herramientas_Estudio/VISUALIZACION_GRADIENT_DESCENT.md` | Al implementar Gradient Descent (cuando ajustes `learning_rate` y criterios de parada) | Ver si “baja” o diverge y por qué |
...
| **Obligatorio** | `Herramientas_Estudio/SIMULACRO_EXAMEN_TEORICO.md` | Tras terminar Chain Rule (antes de saltar a M05/M07) | Verificar que puedes derivar sin mirar apuntes |
...
#### 14) Recursos
- `Herramientas_Estudio/VISUALIZACION_GRADIENT_DESCENT.md`.
...
## A.1 Contexto Histórico: El Cálculo como Lenguaje del Cambio

### Los Fundadores

- **Isaac Newton (1665):** Desarrolló "fluxiones" para física
- **Gottfried Leibniz (1684):** Notación moderna ($\frac{df}{dx}$)
- **Augustin-Louis Cauchy (1821):** Rigor formal con límites
- **Karl Weierstrass (1860s):** Definición épsilon-delta

### Por Qué Importa para ML

El **aprendizaje** en ML es fundamentalmente **optimización**:

$$
\theta^* = \arg\min_\theta \mathcal{L}(\theta; \mathcal{D})
$$

El cálculo multivariante proporciona las herramientas para navegar espacios de parámetros de millones de dimensiones.

---

## A.2 Analogía de Alto Impacto: El Escalador Ciego en Montaña

Imagina un escalador con los ojos vendados en una montaña:

- **No puede ver** el panorama completo
- **Sí puede sentir** la inclinación del terreno bajo sus pies
- **Estrategia:** Dar pasos pequeños en la dirección de máximo descenso

**El gradiente es el "sentido del tacto" del optimizador.**

> En un espacio de 175 mil millones de parámetros (GPT-3), el gradiente es la única brújula viable.

---

## A.3 Definición Formal: Derivada y Diferenciabilidad

### Derivada en $\mathbb{R}$

$$
f'(x) = \lim_{h \to 0} \frac{f(x+h) - f(x)}{h}
$$

### Derivadas Parciales en $\mathbb{R}^n$
...
## A.4 Derivada Direccional

### Definición

La derivada de $f$ en dirección del vector unitario $\mathbf{u}$:
...
- **Sin cambio:** Perpendicular a $\nabla f$ (curva de nivel)

---

## A.5 La Jacobiana y la Hessiana

### Jacobiana: Derivadas de Funciones Vectoriales

Para $\mathbf{f}: \mathbb{R}^n \to \mathbb{R}^m$:
...
| Eigenvalues de $H$ | Tipo de Punto |
|--------------------|---------------|
| Todos $> 0$ | Mínimo local |
| Todos $< 0$ | Máximo local |
| Mixtos | Punto silla |

---

## A.6 Regla de la Cadena: Forma General
...
Para $f: \mathbb{R}^m \to \mathbb{R}$ y $\mathbf{g}: \mathbb{R}^n \to \mathbb{R}^m$:

$$
\nabla_{\mathbf{x}} (f \circ \mathbf{g}) = J_{\mathbf{g}}^T \cdot \nabla_{\mathbf{g}} f
$$

### Backpropagation como Chain Rule

Para una red $L = \ell(f_L(f_{L-1}(\cdots f_1(\mathbf{x}))))$:

$$
\frac{\partial L}{\partial \mathbf{W}_1} = \frac{\partial L}{\partial \mathbf{a}_L} \cdot \frac{\partial \mathbf{a}_L}{\partial \mathbf{a}_{L-1}} \cdots \frac{\partial \mathbf{a}_2}{\partial \mathbf{a}_1} \cdot \frac{\partial \mathbf{a}_1}{\partial \mathbf{W}_1}
$$

---

## A.7 Algoritmos de Optimización: Análisis Teórico

### Gradient Descent: Convergencia

Para funciones $L$-Lipschitz suaves con learning rate $\eta \leq 1/L$:

$$
f(\mathbf{x}_T) - f(\mathbf{x}^*) \leq \frac{\|\mathbf{x}_0 - \mathbf{x}^*\|^2}{2\eta T}
$$

**Tasa de convergencia:** $O(1/T)$
...
**Ventaja:** Complejidad $O(1)$ por iteración vs $O(n)$ para batch
...
Combina momentum con normalización adaptativa:
...
|-------------------------|-------------------|-------------------|
| Gradiente | DTSA 5509 (Supervised) | Entrenamiento de modelos |
| Chain Rule | DTSA 5511 (Deep Learning) | Backpropagation |
| Hessiana | DTSA 5509 | Métodos de segundo orden |
| Adam/SGD | DTSA 5511 | Optimización de redes profundas |
| Jacobiana | DTSA 5511 | Análisis de transformaciones |
...
3. **Goodfellow, I., Bengio, Y., & Courville, A. (2016).** *Deep Learning*, Chapter 4. MIT Press.
...
|----------|--------|-----------|
| [M02_Algebra_Lineal](../../M02_Algebra_Lineal/) | [README](../../README.md) | [M04_Probabilidad_Estadistica](../../M04_Probabilidad_Estadistica/) |
