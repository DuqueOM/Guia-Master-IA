...
- Regresión lineal (táctil):
  - `streamlit run M05_supervised/visualizacion_regresion.py`
- Overfitting / Bias–Variance (grado 1→20):
  - `streamlit run M05_supervised/overfitting_bias_variance_app.py`
...
- Evaluación (rúbrica): [Herramientas_Estudio/RUBRICA_v1.md](../Herramientas_Estudio/RUBRICA_v1.md) (scope `M05` en `rubrica.csv`)
...
| **Obligatorio** | `Herramientas_Estudio/DIRTY_DATA_CHECK.md` | Antes del primer entrenamiento real (Semana 9–10), al preparar datasets | Evitar que el modelo “aprenda basura” por fallas de datos |
| **Obligatorio** | `Herramientas_Estudio/DIARIO_ERRORES.md` | Cada vez que veas métricas incoherentes, accuracy “mágico” o divergencia | Registrar bugs, causas y fixes reproducibles |
...
- **v4.0:** usa `Herramientas_Estudio/SIMULACRO_EXAMEN_TEORICO.md` para preguntas tipo examen (sigmoid vs softmax, BCE vs MSE).
...
- [PLAN_V4_ESTRATEGICO.md](../../Recursos_Adicionales/Planes_Estrategicos/PLAN_V4_ESTRATEGICO.md) (rutina + simulacros)
- [PLAN_V5_ESTRATEGICO.md](../../Recursos_Adicionales/Planes_Estrategicos/PLAN_V5_ESTRATEGICO.md) (validación externa / rigor)
...
- v4.0: usa simulacros para preguntas tipo examen (`Herramientas_Estudio/SIMULACRO_EXAMEN_TEORICO.md`).
...
- `M08_Proyecto_Integrador/Notebooks/decision_tree_from_scratch.py`

Ejecuta:

```bash
python3 M08_Proyecto_Integrador/Notebooks/decision_tree_from_scratch.py --criterion gini --max-depth 5
```
...
- `M08_Proyecto_Integrador/Notebooks/decision_tree_from_scratch.py` (árbol de decisión simple desde cero, sin gradientes).
...
> Esta sección complementa el contenido práctico con rigor matemático formal, contexto histórico y conexiones teóricas profundas.

---

## A.1 Contexto Histórico

### Evolución del Aprendizaje Supervisado

- **1805:** Legendre & Gauss — Método de mínimos cuadrados
- **1936:** Fisher — Análisis discriminante lineal (LDA)
- **1958:** Rosenblatt — Perceptrón
- **1963:** Vapnik & Chervonenkis — Teoría VC
- **1995:** Vapnik & Cortes — Support Vector Machines
- **2001:** Breiman — Random Forests

### El Framework Estadístico del Aprendizaje

El aprendizaje supervisado se formaliza como **aproximación de funciones**:

> Dado un dataset $\mathcal{D} = \{(x_i, y_i)\}_{i=1}^{n}$, encontrar $f: \mathcal{X} \to \mathcal{Y}$ que generalice a datos no vistos.

---

## A.2 Analogía de Alto Impacto: El Estudiante que Estudia para el Examen

- **Training set:** Ejercicios resueltos del libro
- **Validation set:** Exámenes de años anteriores
- **Test set:** El examen real (nunca visto)

**Dos extremos peligrosos:**
1. **Memorizar respuestas** (overfitting): 100% en ejercicios, 30% en examen
2. **No estudiar suficiente** (underfitting): 50% en todo

**Objetivo:** Aprender los **patrones subyacentes**, no las respuestas específicas.

---

## A.3 Formalización: Riesgo y Minimización Empírica

### Riesgo Real (Poblacional)

$$R(f) = \mathbb{E}_{(x,y) \sim P}[\mathcal{L}(f(x), y)]$$

**Problema:** No conocemos la distribución $P$.

### Riesgo Empírico

$$\hat{R}(f) = \frac{1}{n}\sum_{i=1}^{n} \mathcal{L}(f(x_i), y_i)$$

### Principio ERM (Empirical Risk Minimization)

$$\hat{f} = \arg\min_{f \in \mathcal{F}} \hat{R}(f)$$

---

## A.4 Regresión Lineal: Interpretación Probabilística

### Modelo Generativo

Asumiendo ruido Gaussiano:

$$y = \mathbf{w}^T \mathbf{x} + \epsilon, \quad \epsilon \sim \mathcal{N}(0, \sigma^2)$$

### MLE = Mínimos Cuadrados

$$\hat{\mathbf{w}}_{\text{MLE}} = \arg\min_{\mathbf{w}} \|\mathbf{y} - X\mathbf{w}\|_2^2 = (X^TX)^{-1}X^T\mathbf{y}$$

---

## A.5 Regresión Logística: Derivación Completa
...
## A.6 Bias-Variance Tradeoff: Descomposición del Error

### Teorema Fundamental

$$\mathbb{E}[(y - \hat{f}(x))^2] = \underbrace{\text{Bias}^2(\hat{f})}_{\text{Error sistemático}} + \underbrace{\text{Var}(\hat{f})}_{\text{Sensibilidad}} + \underbrace{\sigma^2}_{\text{Ruido irreducible}}$$
...
## A.7 Regularización: Interpretación Bayesiana

### Ridge = Prior Gaussiano

$$\hat{\theta}_{\text{MAP}} = \arg\max_\theta [\log P(\mathcal{D}|\theta) + \log P(\theta)]$$

Con $P(\theta) \propto \exp(-\lambda\|\theta\|_2^2)$:

$$\hat{\theta}_{\text{Ridge}} = (X^TX + \lambda I)^{-1}X^T\mathbf{y}$$

### Lasso = Prior Laplaciano

Con $P(\theta) \propto \exp(-\lambda\|\theta\|_1)$: produce **sparsity**.

---

## A.8 Support Vector Machines: Formulación Dual
...
| Kernel | Fórmula |
|--------|---------|
| Lineal | $\mathbf{x}^T \mathbf{x}'$ |
| RBF | $\exp(-\gamma\|\mathbf{x} - \mathbf{x}'\|^2)$ |
| Polinomial | $(\mathbf{x}^T \mathbf{x}' + c)^d$ |

---

## A.9 Árboles y Ensembles: Teoría

### Reducción de Varianza por Bagging

Para $B$ modelos i.i.d. con varianza $\sigma^2$:

$$\text{Var}(\bar{f}) = \frac{\sigma^2}{B}$$

### Random Forests: Decorrelación

Al muestrear features, reducimos correlación entre árboles → menor varianza del ensemble.

### Gradient Boosting: Descenso Funcional
...
## A.10 Conexiones con MS-AI Pathway

| Concepto | Curso | Aplicación |
|----------|-------|------------|
| Regresión Lineal/Logística | DTSA 5509 | Modelos baseline |
| Regularización | DTSA 5509 | Control de overfitting |
| Bias-Variance | DTSA 5509/5510 | Selección de modelos |
| SVM | DTSA 5509 | Clasificación con kernels |
| Random Forest/XGBoost | DTSA 5509 | Competencias Kaggle |
| Métricas | Todos | Evaluación de modelos |
...
2. **Bishop, C.M. (2006).** *Pattern Recognition and Machine Learning*, Chapters 3-7.
...
|----------|--------|-----------|
| [M04_Probabilidad_Estadistica](../../M04_Probabilidad_Estadistica/) | [README](../../README.md) | [M06_Aprendizaje_No_Supervisado](../../M06_Aprendizaje_No_Supervisado/) |
