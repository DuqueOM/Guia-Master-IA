...
- Registro de errores: `Herramientas_Estudio/DIARIO_ERRORES.md`
- Evaluación (rúbrica): [Herramientas_Estudio/RUBRICA_v1.md](../Herramientas_Estudio/RUBRICA_v1.md) (scope `M04` en `rubrica.csv`; incluye PB-8)

### Recursos (cuándo usarlos)

| Prioridad | Recurso | Cuándo usarlo en este módulo | Para qué |
|----------|---------|------------------------------|----------|
| **Obligatorio** | `Herramientas_Estudio/DIARIO_ERRORES.md` | Cada vez que aparezca `NaN`, `inf`, `log(0)` u overflow/underflow | Registrar el caso y crear un “fix” reproducible |
...
- **Ejecutable:**
  - `python3 M08_Proyecto_Integrador/Notebooks/gmm_3_gaussians_contours.py`
- **Entregable:**
...
- [ ] Ejecuté `M08_Proyecto_Integrador/Notebooks/gmm_3_gaussians_contours.py` y entiendo los contornos de componentes vs mezcla
...
## A.1 Contexto Histórico: El Nacimiento de la Probabilidad

### Los Fundadores

- **Blaise Pascal & Pierre de Fermat (1654):** Correspondencia sobre juegos de azar — nacimiento formal
- **Jacob Bernoulli (1713):** *Ars Conjectandi* — Ley de los grandes números
- **Thomas Bayes (1763):** Teorema publicado póstumamente
- **Andrey Kolmogorov (1933):** *Grundbegriffe* — Axiomatización moderna
- **Claude Shannon (1948):** *A Mathematical Theory of Communication*

### Por Qué Probabilidad es Central en ML

Machine Learning opera fundamentalmente con **incertidumbre**:

1. **Incertidumbre en datos:** Ruido de medición, datos faltantes
2. **Incertidumbre en modelos:** ¿Cuál es el modelo "correcto"?
3. **Incertidumbre en predicciones:** Intervalos de confianza, no valores únicos

> **ML moderno = optimizar funciones de pérdida derivadas de principios probabilísticos**

---

## A.2 Analogía de Alto Impacto: El Meteorólogo

Un meteorólogo no dice "mañana lloverá" — dice "hay 70% de probabilidad de lluvia".

**¿Por qué?**
- La atmósfera es un sistema caótico
- Los datos de entrada tienen errores
- El modelo es imperfecto

**Igual en ML:**
- No predecimos "esta imagen es un gato"
- Predecimos "$P(\text{gato}|\text{imagen}) = 0.92$"

La **calibración** de estas probabilidades es crítica para tomar decisiones.

---

## A.3 Axiomas de Kolmogorov (Formalización Completa)

### Espacio de Probabilidad

Un **espacio de probabilidad** es una tripleta $(\Omega, \mathcal{F}, P)$:

- $\Omega$: Espacio muestral (todos los resultados posibles)
- $\mathcal{F}$: σ-álgebra de eventos (subconjuntos medibles de $\Omega$)
- $P$: Función de probabilidad $P: \mathcal{F} \to [0, 1]$

### Los Tres Axiomas

1. **No negatividad:** $P(A) \geq 0$ para todo evento $A \in \mathcal{F}$
2. **Normalización:** $P(\Omega) = 1$
3. **Aditividad contable:** Para eventos disjuntos $A_1, A_2, \ldots$:
...
## A.4 Teorema de Bayes: Derivación Rigurosa
...
Despejando $P(A \cap B)$ y sustituyendo:
...
|---------|----------|-------------|
| **Prior** | $P(\theta)$ | Creencia antes de ver datos |
| **Likelihood** | $P(\mathcal{D}|\theta)$ | Probabilidad de datos dado modelo |
| **Posterior** | $P(\theta|\mathcal{D})$ | Creencia después de ver datos |
| **Evidence** | $P(\mathcal{D})$ | Constante de normalización |

---

## A.5 Maximum Likelihood Estimation: Teoría Completa
...
| **Eficiencia asintótica** | Alcanza el límite de Cramér-Rao |
| **Normalidad asintótica** | $\sqrt{n}(\hat{\theta}_n - \theta^*) \xrightarrow{d} \mathcal{N}(0, I(\theta^*)^{-1})$ |
| **Invarianza** | Si $\hat{\theta}$ es MLE de $\theta$, entonces $g(\hat{\theta})$ es MLE de $g(\theta)$ |

### Conexión con Cross-Entropy

**Minimizar Cross-Entropy Loss = Maximizar Log-Likelihood**

$$\mathcal{L}_{\text{CE}} = -\sum_{i=1}^{n} \log p(y_i | x_i, \theta) = -\ell(\theta)$$

---

## A.6 Teoría de la Información: Fundamentos

### Entropía de Shannon

$$H(X) = -\sum_{x} p(x) \log p(x) = \mathbb{E}[-\log p(X)]$$

**Interpretación:** Número promedio de bits necesarios para codificar $X$.
...
$$D_{\text{KL}}(p \| q) = \sum_{x} p(x) \log \frac{p(x)}{q(x)} = H(p, q) - H(p) \geq 0$$

**Propiedades:**
- $D_{\text{KL}}(p \| q) = 0$ si y solo si $p = q$
- **No simétrica:** $D_{\text{KL}}(p \| q) \neq D_{\text{KL}}(q \| p)$
...
## A.7 Ley de los Grandes Números y Teorema Central del Límite

### LGN (Débil)

Si $X_1, X_2, \ldots$ son i.i.d. con $\mathbb{E}[X_i] = \mu$:
...
**Implicación:** La distribución de la media muestral es aproximadamente Normal para $n$ grande, independientemente de la distribución original.

---

## A.8 Conexiones con Cursos del MS-AI Pathway

| Concepto de Este Módulo | Curso del Pathway | Aplicación Directa |
|-------------------------|-------------------|-------------------|
| Teorema de Bayes | DTSA 5509 (Supervised) | Naive Bayes, inferencia |
| MLE | DTSA 5509 | Entrenamiento de modelos |
| Cross-Entropy / KL | DTSA 5511 (Deep Learning) | Funciones de pérdida |
| Distribuciones | DTSA 5510 (Unsupervised) | GMM, modelos generativos |
| Markov Chains | DTSA 5512 | Modelos secuenciales |

---

## A.9 Referencias Académicas

1. **Kolmogorov, A.N. (1933).** *Grundbegriffe der Wahrscheinlichkeitsrechnung*.
2. **Bishop, C.M. (2006).** *Pattern Recognition and Machine Learning*, Chapters 1-2.
3. **Murphy, K.P. (2012).** *Machine Learning: A Probabilistic Perspective*, Chapters 2-5.
